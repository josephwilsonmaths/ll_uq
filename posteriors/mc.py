...
class MCDropout(object):
    ...
    def test(self, loader, samples, verbose=False, extra_verbose=False):
        self.network.train()
        ...
        for s in pbar:
            ...
            if extra_verbose:
                inner_pbar = tqdm.tqdm(loader)
            else:
                inner_pbar = loader
            
            for x,_ in inner_pbar:
    ...
    def mean_variance(self, loader, samples, verbose=False, extra_verbose=False):
        predictions = self.test(loader, samples, verbose, extra_verbose).cpu()
