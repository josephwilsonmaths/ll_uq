...
class SGLDClassification(SGLDBase):
    ...
    def training_step(
        ...
        """
        sgld_opt = self.optimizers()
        self.lr_decay(sgld_opt)

        # print(f'lr: {sgld_opt.param_groups[0]['lr']:.3}')
        ...
        self.log(
            "train_loss", loss, batch_size=batch[self.input_key].shape[0], prog_bar=True
        )  # logging to Logger
        self.train_metrics(self.adapt_output_for_metrics(out), y)
    ...
    def predict_step(
        self, X: Tensor, batch_idx: int = 0, dataloader_idx: int = 0, device = 'cpu'
    ) -> dict[str, Tensor]:
        """Predict step with SGLD, take n_sgld_sampled models, get mean and variance.
